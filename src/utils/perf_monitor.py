...
class PerformanceMonitor:
    ...
    def create_embed(self, stats: dict) -> Embed:
        ...
        embed.add_field(name="API Latency", value=f"`{stats['latency_ms']} ms`", inline=True)
        embed.add_field(name="CPU Usage", value=f"`{stats['cpu_percent']} %`", inline=True)
        embed.add_field(name="RAM Usage", value=f"`{stats['ram_usage_mb']:.1f} MB`", inline=True)

        leak_msg = ""
        if stats["leaks"]:
            for leak in stats["leaks"]:
                leak_msg += f"`+{leak['size_diff_kb']:.1f} KiB`\n└ `{leak['traceback']}`\n"
        else:
            leak_msg = "No leaks have been detected since the last check."

        embed.add_field(name="Trace of Memory leaks (Top 3, vs. previous)", value=leak_msg, inline=False)
        return embed
