...
class StatsCog(commands.Cog):
    ...
    async def on_message(self, message: Message):
        ...
        if self.bot.stats_db:
            await self.bot.stats_db.cache_message(
    ...
    async def leaderboard(self, interaction: Interaction):
        ...
        if self.db:
            ...
            embed = Embed(title=self.bot.internal_translator.T(
                "command.stats.top.embeds.default.title", loc), color=Color.gold())
            description = ""
    ...
    @group.command(name="wordcloud", description=app_commands.locale_str("command.stats.wordcloud.description"))
    async def wordcloud(self, interaction: Interaction, limit: int = 1000):
        ...
        if not self.bot.stats_db:
            return await interaction.followup.send(
                embed=Embed(
                    title=self.bot.internal_translator.T("error.embeds.database_not_available.title", loc),
                    description=self.bot.internal_translator.T("error.embeds.database_not_available.description", loc),
                    color=Color.red(),
        ...
        if not interaction.channel:
            return await interaction.followup.send(
                embed=Embed(
                    title=self.bot.internal_translator.T("error.embeds.unsupported_channel_type.title", loc),
                    description=self.bot.internal_translator.T("error.embeds.unsupported_channel_type.description", loc),
                    color=Color.red(),
                    timestamp=datetime.now(UTC)
                )
            )

        rows = await self.bot.stats_db.get_cached_messages(interaction.channel.id, limit)

        if not rows:
            return await interaction.followup.send(
                embed=Embed(
                    title=self.bot.internal_translator.T("error.embeds.data_not_found.title", loc),
                    description=self.bot.internal_translator.T("error.embeds.data_not_found.description", loc),
                    color=Color.red(),
                    timestamp=datetime.now(UTC)
                )
            )
            ...
            if re.search(r'[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]', content):
                for token in self.tokenizer.tokenize(content):
                    ...
                    if pos_str and surface:
                        main_pos = pos_str.split(',')[0]

                        if main_pos == '名詞' and len(surface) > 1 and not surface.isspace() and not surface.isdigit():
                            valid_words.append(surface)

            words_tokens = word_tokenize(content.lower())
            for word in words_tokens:
                if (
                    len(word) > 1 and word.isalnum() and
                    word not in self.eng_stopwords and
                    word not in self.spa_stopwords and
                    word not in self.rus_stopwords and
                    not re.search(r'[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]', word)
                ):
        ...
        try:
            # 3. フォントの選定 (日本語・ロシア語に対応したフォントを指定)
            font_path = "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc"
            if not os.path.exists(font_path):
                font_path = "/usr/share/fonts/truetype/fonts-japanese-gothic.ttf"

            # 画像生成処理を別スレッドで実行してフリーズを回避
            def generate_wc():
                wc = WordCloud(
                    font_path=font_path,
                    width=900,
            ...
            await interaction.followup.send(
                content=f"📊 PostgreSQLのキャッシュから過去 **{len(rows)}** 件を解析！多言語対応ワードクラウドだ〜！",
                file=discord_file
            )

        except Exception as e:
            await interaction.followup.send(f"画像生成でエラーが発生しちゃった D:< \nエラー内容: `{e}`")

    @group.command(name="active_pattern", description="Generates a graph of messages per hour")
    @app_commands.describe(target="Specify if you want to specify member (Use empty for whole channel)")
    async def active_pattern_command(self, interaction: Interaction, target: Member | None = None):
        loc = await self.bot.settings_db.get_locale(interaction) if self.bot.settings_db else interaction.locale.value
        ...
        rows = await self.bot.stats_db.get_active_pattern(interaction.channel.id, target_id)
        ...
        def generate_chart():
            ...
            bars = ax.bar(hours, counts, color='#5865F2', edgecolor='#7289DA', alpha=0.8, width=0.7)

            ax.set_title("Chat Activity Pattern (24h)", fontsize=14, pad=15, color='white', fontweight='bold')
        ...
        try:
            ...
            title_name = target.display_name if target else f"#{interaction.channel.name}"
