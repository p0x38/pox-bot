...
class ContributorsCog(Cog):
    ...
    group = app_commands.Group(name="contributors",
    ...
    @group.command(name="list", description=app_commands.locale_str(
        "Lists all contributors.", extras={"key": "command.contributors.list.description"}))
    async def list_contributors(self, interaction: Interaction):
        ...
        embed = Embed(
            title=self.bot.internal_translator.T("command.contributors.list.embeds.default.title", loc),
            description=self.bot.internal_translator.T(
                "command.contributors.list.embeds.default.description", loc))
        ...
        for contributor in self.contributor_list:
            ...
            name = contributor.get("name", self.bot.internal_translator.T("text.unknown", loc))
            contribution = contributor.get("description",
    ...
    async def contributor_autocomplete(self, interaction: Interaction, current: str) -> list[  # noqa: ARG002
    ...
    @group.command(name="view", description=app_commands.locale_str(
        "Shows contributor.", extras={"key": "command.contributors.view.description"}))
    ...
    async def view_contributor(self, interaction: Interaction, contributor_id: str):
        ...
        embed = Embed(title=self.bot.internal_translator.T(
            "command.contributors.view.embeds.default.title", loc, {"contributor": contributor_id}))

        await interaction.response.defer()

        contributor_data = next(
            (d for d in self.contributor_list if d.get("command") == contributor_id), None)

        if contributor_data:
            embed.set_footer(text=contributor_data.get("quote"))

            rows = [
                contributor_data.get("description",
                                     self.bot.internal_translator.T("text.unknown", loc)), "\n\n",
            ]
            ...
            embed.title = self.bot.internal_translator.T("error.embeds.contributor_not_found.title", loc)
            embed.title = self.bot.internal_translator.T(
                "error.embeds.contributor_not_found.description", loc,
