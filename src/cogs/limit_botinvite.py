...
class BotInvitationLimiterCog(commands.Cog):
    ...
    async def guild_watchdog(self):
        if len(self.bot.guilds) > self.max_servers:
            ...
            sorted_guilds = sorted(self.bot.guilds, key=lambda g: g.me.joined_at or fallback_date,
                                   reverse=True)
    ...
    async def on_guild_join(self, guild: Guild):
        if len(self.bot.guilds) > self.max_servers:
            logger.warning(f"Joined {guild.name}, but limit reached ({len(self.bot.guilds)}/{
                self.max_servers}). Leaving...")
