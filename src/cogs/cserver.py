    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        if interaction.user != self.user:
            await interaction.response.send_message(
                "Your not executor of this menu idiot.", ephemeral=True)
            return False
        return True
...
class GuildCog(commands.Cog):
    ...
    checker_group = app_commands.Group(
        name="guild", description=app_commands.locale_str("command.guild.description"))

    @checker_group.command(
        name="info", description=app_commands.locale_str("command.guild.info.description"))
    ...
    async def check_server_info(self, interaction: Interaction):
        ...
        if guild:
            temp1 = {
                ...
                'server_large': "Yes" if guild.large else "No",
            ...
            e.set_author(name=f"By {guild.owner.name if guild.owner else 'Unknown'}",
                         icon_url=(
                             guild.owner.avatar.url
                             if guild.owner and guild.owner.avatar else None
                             ))
    ...
    @checker_group.command(
        name="kickmembers",
        description=app_commands.locale_str("command.guild.kickmembers.description"))
    @app_commands.guild_only()
    ...
    @checker_group.command(
        name="listmembers",
        description=app_commands.locale_str("command.guild.listmembers.description"))
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    ...
    async def list_all_realusers(self, interaction: Interaction, sort_by: str | None = None):
        ...
        if not final_texts:
            embed.description = self.bot.internal_translator.T("error.embeds.no_members.description", loc)
            embed.title = self.bot.internal_translator.T("error.embeds.no_members.title", loc)
            return await interaction.followup.send(embed=embed)
    ...
    @checker_group.command(
        name="nsfw_level",
        description=app_commands.locale_str("command.guild.nsfw_level.description"))
    @app_commands.guild_only()
    async def check_nsfw_level(self, interaction: Interaction):
        ...
        if interaction.guild:
            embed.description = self.bot.internal_translator.T(f"text.NSFWLevel.{interaction.guild.nsfw_level.name}", loc)
        else:
            return await interaction.followup.send("Guild not found.")
    ...
    @cached(300)
    @checker_group.command(
        name="icon", description=app_commands.locale_str("command.guild.icon.description"))
    @app_commands.guild_only()
    ...
    @checker_group.command(
        name="count", description=app_commands.locale_str("command.guild.count.description"))
    @app_commands.guild_only()
        ...
        if not interaction.guild:
            return await interaction.followup.send(
                embed=Embed(title="Error",
                            description=locale_str("error.GuildOnly"), color=Color.red()))
    ...
    @checker_group.command(
        name="members_list",
        description=app_commands.locale_str("command.guild.members_list.description"))
    @app_commands.guild_only()
    async def get_members(self, interaction: Interaction, include_bot: bool = False,
                          include_member: bool = True):
        await interaction.response.defer()

        if not interaction.guild:
            return await interaction.followup.send(
                embed=Embed(title="Error", description=locale_str("error.GuildOnly"),
                            color=Color.red()))
        ...
        if not include_bot and not include_member:
            embed.color = Color.red()
            embed.description = "You cannot exclude both!"
            return await interaction.followup.send(embed=embed)

        for member in sorted(interaction.guild.members,
                             key=lambda r: r.top_role.position, reverse=True):
    ...
    @checker_group.command(
        name="search_members",
        description=app_commands.locale_str("command.guild.search_members.description"))
    ...
    async def get_roles(self, interaction: Interaction):
        ...
        embed = Embed(
            ...
            description="\n".join([role.mention for role in sorted(
    ...
    async def fetch_guild_history(self, guild: Guild):
        ...
        async for entry in guild.audit_logs(limit=128):
            ...
            if entry.action == AuditLogAction.kick:
                history['kicks'].append(
                    f"**Target:** {target} | **Mod:** {user}\n- *Reason: {reason}*")
            elif entry.action == AuditLogAction.ban:
                history['bans'].append(
                    f"**Target:** {target} | **Mod:** {user}\n- *Reason: {reason}*")
            elif entry.action == AuditLogAction.member_update and hasattr(entry.after,
                'communication_disabled_until') and entry.after.communication_disabled_until:
                until = entry.after.communication_disabled_until.strftime("%Y-%m-%d %H:%M")
                history["timeouts"].append(
                    f"**Target:** {target} | **Until:** {until}\n- *Reason: {reason}*")

        return history

    @checker_group.command(
        name="logs", description=app_commands.locale_str("command.guild.logs.description"))
