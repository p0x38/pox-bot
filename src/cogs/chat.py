        ...
        while True:
            with patch_stdout():
                ...
                if content.startswith('/'):
                    match (args[0]):
                        ...
                        case '/channels':
                            for channel in self.bot.get_all_channels():
                                if channel.permissions_for(channel.guild.me).send_messages:
                                    logger.info(
                                        f"[{channel.id}]: {channel.guild.name} - #{channel.name}"
                                    )
                        case '/servers':
                        ...
                        case '/info':
                            rows_to_add = {
                                ...
                                'Current channel': ((
                                        f"[{self.current_channel.guild.id}/{self.current_channel.id}]: "
                                        f"{self.current_channel.guild.name} - #{self.current_channel.name}")
                    ...
                    if self.current_channel and isinstance(
                        self.current_channel, discord.TextChannel
                    ):
                        try:
