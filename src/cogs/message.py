...
class DMSendModal(discord.ui.Modal):
    def __init__(self, enable_sent_by: bool | None, member) -> None:
        ...
        self.text_to_send = discord.ui.TextInput(label="Text to send", style=TextStyle.paragraph,
                                                 required=True)
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            ...
            if self.member.id == 1321324137850994758 and not sent_by_text:
                ...
                await interaction.response.send_message("Your message sent as DM.", ephemeral=True)
...
class MessageCog(commands.Cog):
    ...
    async def purge_messages(self, interaction: Interaction, limit: int | None = 100):
        ...
        if isinstance(interaction.channel, discord.TextChannel):
            deleted = await interaction.channel.purge(limit=limit if limit is not None else 100,
    ...
    async def delete_messages_sent_by_bot(self, ctx: Interaction, limit: int = 100):
        ...
        if isinstance(ctx.channel, discord.abc.GuildChannel):
            ...
            await ctx.response.send_message(
                content=f"Deleted {deleted_count} messages including the replies.")
    ...
    @app_commands.guild_only()
    async def send_dm_to_member(self,
                                ctx: Interaction, member: Member, enable_sent_by: bool | None):
    ...
    async def send2(self, interaction: Interaction, message: str):
        ...
        for guild in guilds:
            ...
            for channel in channels:
                ...
                if isinstance(channel, TextChannel) and re.search(
                    r"[a-zA-Z0-9_\-\s]", channel.name):
                    total += 1
                    try:
    ...
    @app_commands.guild_only()
    async def search_messages_in_channel(self, interaction: Interaction,
                                         keyword: str, limit: int | None = 100):
        await interaction.response.defer()
        found_messages = []

        limit = 1000 if limit is None else stuff.clamp(limit, 1, 10000)

        if isinstance(interaction.channel, discord.TextChannel):
            async for message in interaction.channel.history(limit=limit):
                if keyword.lower() in message.content.lower():
                    logger.debug(f"Found message: {message.content} by {message.author.name}")
                    found_messages.append(f"- {message.author.name}: {stuff.crop_word(
                        message.content, keyword) or textwrap.shorten(
                            message.content, width=30)} (ID: {message.id})")
    ...
    async def fetch_last_message(self, interaction: Interaction):
        ...
        if message:
            embed.title = f"Last Message sent in {(
                ...
                else 'Not an Text Channel')} by {message.author.name}"
    ...
    @cached(60)
    @group.command(name="get_random_message",
