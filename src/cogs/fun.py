...
class FunCog(Cog):
    ...
    group = app_commands.Group(name="fun",
                               description=app_commands.locale_str("command.fun.description"))

    @group.command(name="guess_game",
                   description=app_commands.locale_str("command.fun.guess_game.description"))
    ...
    @group.command(name='guess',
                   description=app_commands.locale_str("command.fun.guess.description"))
    async def make_guess(self, interaction: Interaction, number: int):
        ...
        if attempts < 16:
            if number < secret_number:
                ...
                return await interaction.response.send_message(
                    f"Congrats {interaction.user.name}. "
                    f"You've guessed my number {secret_number} in just {attempts} attempts.\n"
                    f"You can play the guessing game by typing `/guess_game`."
                )
        else:
            special = ""
            ...
            return await interaction.response.send_message(
                f"You failed to guess. "
                f"{special}\n It was {secret_number}. "
                f"Your score is {game_data['score']}. Try again."
            )

    @group.command(
        name="stopguess", description=app_commands.locale_str("command.fun.stopguess.description"))
    ...
    @group.command(
        ...
        description=app_commands.locale_str("command.fun.job_application.description"))
    ...
    @group.command(
        name="boop_member",
        description=app_commands.locale_str("command.fun.boop_member.description"))
    ...
    @app_commands.command(
        name="ship", description=app_commands.locale_str("command.fun.ship.description"))
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def ship_users(self, interaction: Interaction,
                         user1: User | Member, user2: User | Member | None = None):
        if not user2:
            if interaction.guild:
                ...
                return await interaction.response.send_message(
                    "The command should've to ran with guild")
