...
class WelcomeCog(commands.Cog):
    ...
    async def send_message(self, member: Member, leaving: bool = False):
        if not self.bot.guild_db:
            return
        if not await self.bot.guild_db.get_config(member.guild.id):
            return

        guild_settings = await self.bot.guild_db.get_config(member.guild.id)
        if not guild_settings.welcome or not guild_settings.welcome.enabled:
            return
        if not isinstance(guild_settings.welcome, WelcomeConfig):
            return
        ...
        if target_channel_id == 0 or not target_channel_id:
            return
    ...
    async def interaction_check(self, interaction) -> bool:
        ...
        if not interaction.guild:
            embed = Embed(
                title=self.bot.internal_translator.T("error.embeds.guild_only.title", loc),
                description=self.bot.internal_translator.T("error.embeds.guild_only.description", loc),
                timestamp=datetime.now(UTC),
                color=Color.red()
            )
    ...
    async def toggle_welcome(self, interaction: Interaction, enabled: bool = False):
        ...
        if guild_settings.features and "welcome" in guild_settings.features and isinstance(
            guild_settings.features["welcome"], WelcomeConfig):
        ...
        embed.title = self.bot.internal_translator.T("command.welcome.toggle.embeds.default.title", loc)
        embed.description = self.bot.internal_translator.T("command.welcome.toggle.embeds.default.description", loc,
    ...
    async def set_joinchannel(self, interaction: Interaction, channel: TextChannel):
        ...
        embed.title = self.bot.internal_translator.T("command.welcome.joinchannel.embeds.default.title", loc)
        embed.description = self.bot.internal_translator.T("command.welcome.joinchannel.embeds.default.description", loc,
    ...
    async def set_leavechannel(self, interaction: Interaction, channel: TextChannel):
        ...
        embed.title = self.bot.internal_translator.T("command.welcome.leavechannel.embeds.default.title", loc)
        embed.description = self.bot.internal_translator.T("command.welcome.leavechannel.embeds.default.description", loc,
                                                           channel=channel.mention)
