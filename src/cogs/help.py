...
def _get_command_signature(
    command: app_commands.Command | app_commands.Group, prefix: str = "") -> str:
    ...
    async def help_command(self, interaction: Interaction, index_number: int | None = 0):
        ...
        if index_number > (round(len(all_commands) / self.commands_per_page) - 1):
            await interaction.response.send_message(
                "Requested search index is larger than list length.", ephemeral=True)
            return

        remaining = len(all_commands) - len(
            all_commands[:(index_number + 1) * self.commands_per_page])
        limited_commands = all_commands[
            (index_number * self.commands_per_page):(index_number + 1) * self.commands_per_page]

        lines = []

        lines.append(f"Index: {index_number} / {
            (round(len(all_commands) / self.commands_per_page) - 1)}")
