...
class RoleCog(commands.Cog):
    ...
    async def permission_autocomplete(self, interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:  # noqa: ARG002
        choices = [
    ...
    async def edit_role_permission(
        ...
        if not interaction.guild.me.guild_permissions.manage_roles:
            return await interaction.response.send_message("I do not have permission to manage roles.")
        if role.position >= interaction.guild.me.top_role.position:
            return await interaction.response.send_message(
                "I do not have permission to manage roles that is higher than my highest roles.")
        if permission not in ALLOWED_PERMISSIONS_TO_EDIT:
            return await interaction.response.send_message(f"Permission ID {permission} is not allowed.", ephemeral=True)
        if not hasattr(Permissions, permission):
            return await interaction.response.send_message(f"Permission {permission} not found.")
