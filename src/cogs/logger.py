...
class LoggerCog(commands.Cog):
            ...
            case InteractionType.application_command:
                ...
                logger.info(
                    r'"{}" used command "{}" at "{}"'.format(
                        ...
                        command_name,
                        f"{guild_name} - #{channel_name}" if interaction.guild else f"{
                            channel_name}"
                    )
                )
            case _:
    ...
    async def on_member_join(self, member: Member):
        logger.info(f"Member {member.display_name} ({member.id}) joined to {member.guild.name} ({
            member.guild.id}).")

    @commands.Cog.listener()
    async def on_member_remove(self, member: Member):
        logger.info(f"Member {member.display_name} ({member.id}) left from {member.guild.name} ({
            member.guild.id}).")
