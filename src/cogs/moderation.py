...
class ModerationCog(commands.Cog):
    ...
    blacklist = app_commands.Group(
        name="blacklist", description=app_commands.locale_str("command.moderation.blacklist.description"), parent=group)
            ...
            for entry in word_cfg.blacklists:
                ...
                match (entry.type):
                    ...
                    case BlacklistEntryMatchType.exact:
                        detected = pattern.lower() == content.lower() if entry.case_insensitive else pattern == content
                    case BlacklistEntryMatchType.whole_word:
                        regex_pattern = rf"\b{re.escape(pattern)}\b"
                        if re.search(regex_pattern, content, flags):
                            detected = True
                    case _:
                        detected = pattern.lower() in content.lower() if entry.case_insensitive else pattern in content
    ...
    async def add_word(
        ...
        if any(entry.trigger == trigger for entry in word_filter.blacklists):
            embed.title = self.bot.internal_translator.T("error.embeds.confliction.title", loc)
            embed.description = self.bot.internal_translator.T("error.embeds.confliction.description", loc, {"value": trigger})
            embed.timestamp = datetime.now(UTC)
            embed.color = Color.red()
        ...
        embed.title = self.bot.internal_translator.T('command.moderation.blacklist.add.embeds.default.title', loc)
        embed.description = self.bot.internal_translator.T('command.moderation.blacklist.add.embeds.default.description', loc, {
            "entry": new_entry.trigger, "id": new_entry.id, "type": new_entry.type})
    ...
    async def remove_word(self, interaction: Interaction, query: str):
        ...
        if not isinstance(wf, WordFilter):
            embed.title = self.bot.internal_translator.T("error.embeds.blacklist_not_configured.title", loc)
            embed.description = self.bot.internal_translator.T("error.embeds.blacklist_not_configured.description", loc)
            embed.timestamp = datetime.now(UTC)
            embed.color = Color.red()
        ...
        if len(wf.blacklists) < original_count:
            ...
            embed.title = self.bot.internal_translator.T('command.moderation.blacklist.remove.embeds.default.title', loc)
            embed.description = self.bot.internal_translator.T('command.moderation.blacklist.remove.embeds.default.description',
                                                               loc, {'query': query})
            return await interaction.followup.send(embed=embed)
        else:
            embed.title = self.bot.internal_translator.T('command.moderation.blacklist.remove.embeds.not_found.title', loc)
            embed.description = self.bot.internal_translator.T(
                'command.moderation.blacklist.remove.embeds.not_found.description', loc, {'query': query})
            return await interaction.followup.send(embed=embed)
    ...
    async def list_words(self, interaction: Interaction):
        ...
        embed.title = self.bot.internal_translator.T('command.moderation.blacklist.list.embeds.default.title', loc)
        ...
        if len(wf.blacklists) != 0:
            ...
            embed.description = self.bot.internal_translator.T(
                "command.moderation.blacklist.list.embeds.no_blacklist.description", loc)

        if len(wf.blacklists) > 20:
            embed.set_footer(text=self.bot.internal_translator.T("command.moderation.blacklist.list.embeds.default.footer", loc,
                                         {"remaining": len(wf.blacklists) - 20}))
