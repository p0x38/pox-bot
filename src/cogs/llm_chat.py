...
class LLMChatCog(commands.Cog):
    ...
    async def format_discord_message(self, message: Message) -> dict[str, Any]:
        ...
        if message_content:
            ...
            message_content = message_content.replace(
                f"<@{self.bot.user.id}> ", "").replace(f"<@!{self.bot.user.id}> ", "")  # type: ignore
        ...
        if formatted_referenced_message_author:
            role_content = f"[{formatted_message_author} -> {
                formatted_referenced_message_author}] {formatted_message_content}"
        else:
            # Priority: @everyone > i were mentioned > normal
            if message.mention_everyone:
                role_content = f"[{formatted_message_author} -> @everyone] {
            ...
            case LLMProviderType.OPENROUTER.value:
                ...
                try:
                    ...
                    if "401" in msg or "Unauthorized" in msg or "invalid api key" in msg.lower():
                        ...
                        raise RuntimeError(
                            "Authentication failed when contacting the LLM provider. Check API key."
                        ) from e
    ...
    async def populate_channel_cache(
        self, channel: (TextChannel | StageChannel | VoiceChannel | Thread | DMChannel |
                        GroupChannel | PartialMessageable), limit: int = 10):
    ...
    async def respond(self,
                      message: Message, provider: str, model: str, include_history: bool = False):
        ...
        try:
            ...
            loop.run_in_executor(None, lambda: open(file_path, "w", encoding="utf-8").write(  # noqa: SIM115
        ...
        if user_cd and now - user_cd < self.cooldown_seconds:
            ...
            with contextlib.suppress(Exception):
                await message.reply(f"You are on cooldown. Please retry after {remaining} seconds",
            ...
            async with message.channel.typing():
                ...
                if output:
                    ...
                    if output.startswith("[REPLY]"):
                        final = output[7:]
                        await message.reply(final[:2000],
                                                   allowed_mentions=AllowedMentions.none())
                    else:
                    ...
                    try:
                        ...
                        to_save = list(self.history[message.channel.id])[-self.history_limit:]
                        loop = asyncio.get_running_loop()
                        loop.run_in_executor(None, lambda: open(  # noqa: SIM115
                            file_path, "w", encoding="utf-8").write(
                                json.dumps(to_save, ensure_ascii=False)))
                    except Exception:
    ...
    async def on_message(self, message: Message):
        ...
        if member is None:
            logger.warning("Could not resolve guild member for bot in guild %s", getattr(
                message.guild, "id", "unknown"))
            return

        permissions = message.channel.permissions_for(member)
        if permissions and permissions.send_messages:
            await self.respond(message, "openrouter", "@preset/default",
