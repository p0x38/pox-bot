...
async def get_reliable_member(guild: Guild, user_id: int) -> tuple[
    MemberFetchStatus, Member | None]:
    """
...
class LeaderboardCog(Cog):
    ...
    @cached(60)
    @group.command(name="pox",
                   description="Shows leaderboard in server who said pox for many times")
    async def poxword_leaderboard(self, interaction: Interaction):
        ...
        if self.bot.db_connection:
            async with self.bot.db_connection.execute(
                "SELECT user_id, pox_count FROM leaderboard ORDER BY pox_count DESC LIMIT 32") as (
                    cursor):
    ...
    async def my_point(self, interaction: Interaction):
        ...
        if self.bot.db_connection:
            async with self.bot.db_connection.execute("SELECT amount FROM words WHERE user_id = ?",
                                                      (interaction.user.id,)) as cursor:
                lbdata = await cursor.fetchone()

            desc = ""

            desc = f"You have {lbdata[0]:,} points." if lbdata else "Nothing you got."
    ...
    async def word_leaderboard(self, interaction: Interaction):
        ...
        if self.bot.db_connection:
            async with self.bot.db_connection.execute(
                "SELECT user_id, amount FROM words ORDER BY amount DESC LIMIT 32") as cursor:
                lbdata = await cursor.fetchall()
    ...
    @cached(120)
    @group.command(name="local_word_points",
                   description="Shows leaderboard in server who has yapping all times")
    @guild_only()
    async def word_leaderboard_local(self, interaction: Interaction):
        await interaction.response.defer()
        if interaction.guild is None:
            await interaction.followup.send("It seems you're not in the server.")
            return

        if self.bot.db_connection:
            message = await interaction.followup.send(
                "Database connection is active; Fetching members...", wait=True)
    ...
    async def user_word_count(self, interaction: Interaction, user: Member):
        ...
        if not self.bot.db_connection:
            return await interaction.followup.send(
                "sorry, bot has no connection with Database.")
