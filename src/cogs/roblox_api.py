...
class RobloxAPICog(commands.Cog):
    ...
    async def roblox_username_autocomplete(self, interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:  # noqa: ARG002
        choices = []
    ...
    async def get_user_avatar(self, interaction: Interaction, username: str):
            ...
            try:
                ...
                if not user:
                    embed.description = self.bot.internal_translator.T(
                        "error.custom.roblox_user_not_found", loc, {"username": username})
                ...
                embed.description = self.bot.internal_translator.T("error.http.400", loc)
                return await interaction.followup.send(embed=embed)
            except Forbidden:
                embed.description = self.bot.internal_translator.T("error.custom.roblox_forbidden", loc)
                return await interaction.followup.send(embed=embed)
            except TooManyRequests:
                embed.description = self.bot.internal_translator.T("error.http.429", loc)
                return await interaction.followup.send(embed=embed)
            except InternalServerError:
                embed.description = self.bot.internal_translator.T("error.http.500", loc)
                return await interaction.followup.send(embed=embed)
        ...
        if avatar_url:
            embed = Embed(title=self.bot.internal_translator.T(
                "command.roblox.avatar.embeds.default.title", loc, {"username": username}))
            embed.set_author(
            ...
            return await interaction.followup.send(
                self.bot.internal_translator.T("error.custom.roblox_failed_retrieve_avatar", loc,
                                               {"username": username}))
            ...
            if isinstance(result, roblox.users.User):
                rows = {
                    ...
                    'label.roblox_user_banned': self.bot.internal_translator.T(
                        "text.boolean.true", loc) if result.is_banned else self.bot.internal_translator.T("text.boolean.false"),
                    'label.roblox_user_created': result.created.strftime("%Y-%m-%d %H:%M:%S UTC")
                }
                embed = Embed(title=self.bot.internal_translator.T("command.roblox.user.embeds.default.title", loc,
                                                          {"username": result.name}), description=result.description)

                for name, value in rows.items():
                    embed.add_field(name=self.bot.internal_translator.T(name, loc), value=value, inline=True)
