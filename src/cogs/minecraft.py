...
class MinecraftCog(commands.Cog):
        ...
        try:
            ...
            try:
                ...
                if status.players.sample:
                    ...
                    row_to_add["minecraft_server_playerlist"] = self.bot.internal_translator.T("text.unknown", loc)
            ...
            embed.title = self.bot.internal_translator.T("error.embeds.minecraft_lookup_failed.title", loc)
            embed.description = self.bot.internal_translator.T("error.embeds.minecraft_lookup_failed.description", loc,
                                                               {"address": address, "e": e})
    ...
    async def user(self, interaction: Interaction, query: str):
        ...
        try:
            ...
            embed.title = self.bot.internal_translator.T("command.minecraft.profile.embeds.default.title", loc,
                                                         {"name": player.name})
            if head:
            ...
            embed.description = self.bot.internal_translator.T("error.embeds.minecraft_user_not_found.description", loc,
                                                               {"query": query})
            await interaction.followup.send(embed=embed)
