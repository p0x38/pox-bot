...
class ChatbotCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.trigger_list = {
            ...
            "shut_up": re.compile(
                r"(?i)\b(shut\s*up|be\s*quiet|stop\s*talking|silence|sybau|stfu)\s*(jim|jim\s*bot|p[o0]x(38)?|stupid|bitch|fucker)\b"
            ),
    ...
    async def on_message(self, message: Message):
        ...
        is_random_lucky = (
            random.random() < self.chat_probability and
            len(message.content) > self.min_message_length
        )
        ...
        if is_mentioned or is_triggered or is_random_lucky:
            ...
            if channel_info['is_locked']:
                logger.warning(
                    "LLM was selected, but other process is running on the channel. aborting..."
                )
                if is_mentioned:
            ...
            try:
                ...
                async with message.channel.typing():
                    ...
                    logger.debug(
                        f"Generating LLM Response from Ollama REST API with prompt: \n\n{context}"
                    )
                    ...
                    if len(self.perf_logs) > self.max_logs:
                        self.perf_logs.pop(0)
                    ...
                    final_text = re.sub(
                        r"^<(\d+|</?assistant>:?|assistant:?|You:?)\s*>:\s*",
                        "",
                        final_text,
                        flags=re.IGNORECASE
                    ).replace("<you>", message.author.mention)

                    final_text = final_text.replace("<user>", message.author.display_name)
                    final_text = final_text.replace("<here>", "@here").replace(
                        "<everyone>", "@everyone"
                    )
                    ...
                    if final_text:
                        ...
                        await message.reply(
                            self.bot.internal_translator.T('messages.chatbot_empty',
                                   message.guild.preferred_locale if message.guild else "en")
                            )
                ...
                if is_mentioned:
                    await message.reply(
                        self.bot.internal_translator.T('messages.chatbot_request_error',
                               message.guild.preferred_locale if message.guild else "en")
                        )
                self.stats["total_failures"] += 1
                ...
                if is_mentioned:
                    await message.reply(
                        self.bot.internal_translator.T('messages.chatbot_response_error',
                               message.guild.preferred_locale if message.guild else "en")
                        )
                self.stats["total_failures"] += 1
            except Exception as e:
                ...
                if is_mentioned:
                    await message.reply(
                        self.bot.internal_translator.T('error.exceptions.Unknown',
                               message.guild.preferred_locale if message.guild else "en",
                               {"e": str(e)})
                        )
                self.stats["total_failures"] += 1
    ...
    @group.command(
        name="add_event",
        description=app_commands.locale_str("command.chatbot.add_event.description")
    )
    ...
    @group.command(
        name="mem",
        description=app_commands.locale_str("command.chatbot.mem.description")
    )
    async def show_memory(self, interaction: Interaction):
        ...
        if not self.history:
            return await interaction.followup.send(
                self.bot.internal_translator.T("command.chatbot.mem.messages.empty", loc))
        ...
        for channel_id, history in self.history.items():
            ...
            channel_name = (
                channel.name if channel and isinstance(channel, TextChannel)
    ...
    @group.command(name="unlock",
                   description=app_commands.locale_str("command.chatbot.unlock.description"))
    async def force_unlock(self, interaction: Interaction):
        ...
        if self.channel_data[channel_id]["is_locked"]:
            ...
            await interaction.response.send_message(
                self.bot.internal_translator.T('command.chatbot.unlock.messages.unlock', loc))
        else:
            await interaction.response.send_message(
                self.bot.internal_translator.T('command.chatbot.unlock.messages.already', loc),
                ephemeral=True)

    @group.command(name="perf",
                   description=app_commands.locale_str("command.chatbot.perf.description"))
    async def show_perfstats(self, interaction: Interaction):
        ...
        try:
            ...
            if not self.perf_logs:
                return await interaction.followup.send(
                    self.bot.internal_translator.T("command.chatbot.perf.messages.empty_logs", loc))

            avg_ttft = sum(l['ttft'] for l in self.perf_logs) / len(self.perf_logs)  # noqa: E741
            avg_tps = sum(l['tps'] for l in self.perf_logs) / len(self.perf_logs)  # noqa: E741
            avg_total = sum(l['total_time'] for l in self.perf_logs) / len(self.perf_logs)  # noqa: E741

            ttft_bar = self.get_latency_bar(avg_ttft, 5.0)
            tps_bar = self.get_latency_bar(10.0 / (avg_tps if avg_tps > 0 else 0.1), 10.0)

            embed = Embed(title=self.bot.internal_translator.T("command.chatbot.perf.embeds.default.title"),
                          color=Color.green())
            embed.description = self.bot.internal_translator.T("command.chatbot.perf.embeds.default.description", loc,
                                       {"generations": len(self.perf_logs)})
            ...
            # Load impact
            high_load = [l['ttft'] for l in self.perf_logs if l['input_len'] > 500]  # noqa: E741
            if high_load:
    ...
    @group.command(name="stats",
                   description=app_commands.locale_str("command.chatbot.stats.description"))
    ...
    @group.command(name="userstats",
                   description=app_commands.locale_str("command.chatbot.user_stats.description"))
    ...
    @group.command(name="clear_memories",
                   description=app_commands.locale_str("command.chatbot.clear_memories.description"))
