...
class LocalizedSettingsView(ui.View):
    def __init__(self, bot: PoxBot, user_id: int, initial_data: SettingsData, db_manager: SettingsDatabase):
    ...
    def create_embed(self) -> Embed:
        ...
        embed.description = (
            ...
            f"**{self.bot.internal_translator.T('settings.fields.locale', loc, {"lang_code": lang_display})}\n\n"
            f"**{self.bot.internal_translator.T('settings.fields.embed_color', loc, {"hex": self.data.embed_color})}"
        )
...
class SettingsCog(commands.Cog):
    ...
    async def show_settings(self, interaction: Interaction):
        ...
        if not self.bot.settings_db:
            embed.description = self.bot.internal_translator.T('error.embeds.no_connection', loc)
        ...
        view = LocalizedSettingsView(self.bot, interaction.user.id, data, self.bot.settings_db)
