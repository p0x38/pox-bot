    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            ...
            if td.total_seconds() <= 0:
                raise ValueError(self.bot.internal_translator.T("error.custom.timeout_duration_lessflow", loc))

            td = min(td, MAX_TIMEOUT)

            if not interaction.guild:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.guild_only", loc))

            if isinstance(interaction.user, User):
                raise TypeError(self.bot.internal_translator.T("error.exceptions.Unknown", loc))

            if self.target == interaction.user:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.tried_to_timeout_himself", loc))

            if self.target == interaction.guild.owner:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.tried_to_timeout_owner", loc))

            if self.target.top_role >= interaction.user.top_role:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.tried_to_timeout_higher", loc))

            if not interaction.guild.me.guild_permissions.moderate_members:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.forbidden_timeout", loc))

            if self.target.top_role >= interaction.guild.me.top_role:
                raise RuntimeError(self.bot.internal_translator.T("error.custom.cannot_timeout_higher", loc))
            ...
            self.embed.description = self.bot.internal_translator.T(
                "messages.timed_out_user", loc, {"user": self.target.mention, "length": td})
...
class DynamicUserInfoView(ui.View):
    ...
    def create_select(self):
        ...
        for key, data in self.categories.items():
            label = self.bot.internal_translator.T(f"modal.DynamicUserInfoView.fields.{key}", self.locale)
        ...
        select = ui.Select(
            placeholder=self.bot.internal_translator.T("modal.DynamicUserInfoView.placeholder", self.locale),
            options=options
        )
    ...
    async def select_callback(self, interaction: Interaction):
        ...
        embed = Embed(
            title=self.bot.internal_translator.T(
                'modal.DynamicUserInfoView.embeds.default.title', interaction.locale, {"category": selected_label}),
        ...
        for field_name, field_value in category_data.get('fields', {}).items():
            ...
            field_name = self.bot.internal_translator.T(f"modal.DynamicUserInfoView.fields.{field_name}", self.locale)
...
class UserCog(commands.Cog):
    def __init__(self, bot):
        ...
        async def contextmenu_kick(interaction: Interaction, member: Member):
            ...
            try:
                ...
                embed.description = self.bot.internal_translator.T("messages.kick_user", loc, {"user": member.display_name})
            except Forbidden:
                embed.description = self.bot.internal_translator.T("error.custom.insufficient_permission_kick", loc,
                                                                   {"user": member.display_name})
            except HTTPException:
                embed.description = self.bot.internal_translator.T("error.exceptions.HTTPException", loc)
            except Exception as e:
                embed.description = self.bot.internal_translator.T("error.exceptions.Unknown", loc, {"e": e})
            finally:
        ...
        async def contextmenu_ban(interaction: Interaction, member: Member):
            ...
            try:
                ...
                embed.description = self.bot.internal_translator.T("messages.ban_user", loc, {"user": member.display_name})
            except Forbidden:
                embed.description = self.bot.internal_translator.T("error.custom.insufficient_permission_ban", loc,
                                                                   {"user": member.display_name})
            except HTTPException:
                embed.description = self.bot.internal_translator.T("error.exceptions.HTTPException", loc)
            except Exception as e:
    ...
    global_group = app_commands.Group(name="user_global", description=app_commands.locale_str("command.user.description"),
    ...
    async def check_how_long_user_has_been(self, interaction: Interaction, member: Member):
        ...
        if not interaction.guild:
            return await interaction.followup.send("You must be run this command in guild mode")
    ...
    async def check_user_info(self, interaction: Interaction, member: Member | User):
        ...
        try:
            ...
            if user:
                e = Embed(title=self.bot.internal_translator.T("command.user.info.embeds.default.title", loc,
                                                               {"user": user.display_name}))
                temp1 = {
                    'user_id': user.id,
                    'user_name': f"`{user.display_name}`",
                    'user_bot': self.bot.internal_translator.T("text.boolean.true" if user.bot else "text.boolean.false", loc),
                    'user_creation': user.created_at.strftime("%Y-%m-%d %H:%M:%S") + f" (<t:{
                        int(user.created_at.timestamp())}:R>)"
                }

                if isinstance(user, Member):
                    roles = [role for role in user.roles if role.name != "@everyone"]
                    temp1['user_highest_role'] = f"<@&{user.top_role.id}>"
                    temp1['user_join'] = user.joined_at.strftime(
                        "%Y-%m-%d %H:%M:%S") + f" (<t:{int(user.joined_at.timestamp())}:R>)" if user.joined_at else (
                            self.bot.internal_translator.T("text.unknown_join", loc))
                    temp1['user_roles'] = ", ".join([f"<@&{role.id}" for role in roles])
                    temp1['user_status'] = format_status(self.bot, user.client_status, loc)
                    temp1['user_nitro'] = user.premium_since.strftime(
                        "%Y-%m-%d %H:%M:%S") if user.premium_since else self.bot.internal_translator.T("label.non_nitro", loc)
                ...
                if self.bot.user_db:
                    ...
                    if data:
                        ...
                        nickname = data.get('nickname') or self.bot.internal_translator.T("text.unknown", loc)
                        description = data.get('description') or self.bot.internal_translator.T(
                            'error.custom.description_not_found', loc)
                        ...
                        if isinstance(activity, Activity):
                            ...
                            match (activity.type):
                                case ActivityType.custom:
                                    info = activity.name
                                case _:
                                    info = self.bot.internal_translator.T(f"text.activity_type.{activity.type.name}", loc,
                                                                          {"activity": activity.name})

                            temp1[f'Activity #{index_activity}'] = f"{info} ({activity.state})"
                        elif isinstance(activity, Game):
                            temp1[f'Activity #{index_activity}'] = self.bot.internal_translator.T("text.activity_type.game",
                                loc, {"activity": activity.name, "platform": activity.platform})
                        elif isinstance(activity, Streaming):
                            temp1[f'Activity #{index_activity}'] = self.bot.internal_translator.T("text.activity_type.stream",
                                loc, {"activity": activity.name, "platform": activity.platform})
                        elif isinstance(activity, CustomActivity):
                            temp1[f'Activity #{index_activity}'] = activity.name
                        elif isinstance(activity, Spotify):
                            temp1[f'Activity #{index_activity}'] = self.bot.internal_translator.T("text.activity_type.spotify",
                                loc, {"title": activity.title, "artist": activity.artist, "album": activity.album})
                        else:
                            temp1[f'Activity #{index_activity}'] = self.bot.internal_translator.T("text.unknown", loc)
                ...
                return await interaction.followup.send(self.bot.internal_translator.T("error.custom.user_not_found", loc))
        except Exception as e:
    ...
    async def set_user_community_profile(self, interaction: Interaction, nickname: str | None = None,
        ...
        embed.title = self.bot.internal_translator.T("command.user.set_profile.embeds.default.title", loc)
        embed.description = self.bot.internal_translator.T("command.user.set_profile.embeds.default.description", loc,
    ...
    async def get_user_avatar(self, interaction: Interaction, member: User | Member):
        ...
        embed = Embed(title=self.bot.internal_translator.T("command.user.avatar.embeds.default.title", loc,
                                                           {"user": member.name}))
        embed.set_image(url=member.display_avatar.url if member.display_avatar else member.default_avatar.url)
        embed.set_footer(text=self.bot.internal_translator.T("command.user.avatar.embeds.default.footer", loc,
    ...
    async def change_nickname(self, interaction: Interaction, member: Member, new_nick: str | None = None):
        ...
        try:
            if new_nick is None:
                await member.edit(nick=None, reason=self.bot.internal_translator.T("command.user.set_nick.reasons.reset", loc,
                                                           {"user": member.mention, "author": interaction.user.name}))
                embed.description = self.bot.internal_translator.T("command.user.set_nick.embeds.reset.description", loc,
                                                                   {"user": member.mention})
                return await interaction.followup.send(embed=embed)
            else:
                await member.edit(nick=None, reason=self.bot.internal_translator.T("command.user.set_nick.reasons.changed", loc,
                                                           {"user": member.mention, "author": interaction.user.name}))
                embed.description = self.bot.internal_translator.T("command.user.set_nick.embeds.changed.description", loc,
                                           {"user": member.mention, "new_nickname": new_nick})
                return await interaction.followup.send(embed=embed)
        except Forbidden:
            embed.title = self.bot.internal_translator.T("error.embeds.missing_permission.title", loc)
            embed.description = self.bot.internal_translator.T("error.embeds.missing_permission.description", loc,
    ...
    async def get_user_status(self, interaction: Interaction, member: Member):
        ...
        e = Embed()
        e.title = self.bot.internal_translator.T("command.user.status.embeds.default.title", loc, {"user": member.display_name})
        e.description = format_status(self.bot, result, loc) if isinstance(result, ClientStatus) else result
        return await interaction.followup.send(embed=e)
    ...
    async def get_remaining_members(self, interaction: Interaction, goal: int | None = None):
        if interaction.guild is None:
            return await interaction.response.send_message("Object is not guild", ephemeral=True)
    ...
    @cached(60)
    @group.command(name="find_first_message_contains",
