...
class RatingCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.file_path = "data/rating.json"
        self.loaded = False
    ...
    async def rate_member(self, interaction: Interaction, member: Member, rate: float, override: bool | None = False):
        if interaction.guild is None:
            return await interaction.response.send_message("You cannot set welcome channel unless the bot is for guild.")
        if member == interaction.user:
            return await interaction.response.send_message("You can't rate yourself.")
        if override is None:
            override = False
        ...
        if interaction.user.id in user_data:
            if not override:
                ...
                return await interaction.response.send_message(
                    f"You have been overwritten the rating for `{member.display_name}` to {rate}!")
    ...
    async def get_rating_info(self, interaction: Interaction, member: Member):
        if interaction.guild is None:
            return await interaction.response.send_message(
                "You cannot set welcome channel unless the bot is for guild.")
        ...
        for rating in sorted_dict.values():
            sum += rating
