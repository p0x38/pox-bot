...
from ....persistence.models import WelcomeChannels, WelcomeConfig
...
class WelcomeCog(commands.Cog):
    ...
    group = app_commands.Group(
        name='welcome',
        description=app_commands.locale_str('command.welcome.description'),
    )
    ...
    async def send_message(self, member: Member, leaving: bool = False):
        if not self.bot.database.guild:
            return
        if not await self.bot.database.guild.get_config(member.guild.id):
            return

        guild_settings = await self.bot.database.guild.get_config(member.guild.id)
        ...
        match leaving:
            case True:
                target_channel_id = (
                    ...
                    if (
                        guild_settings.welcome.channels.leave
                        and guild_settings.welcome.channels.leave != 0
                    )
                    else (
                        guild_settings.welcome.channels.join
                        if (
                            guild_settings.welcome.channels.join
                            and guild_settings.welcome.channels.join != 0
                        )
                        else 0
                    )
                )
        ...
        try:
            ...
            rows_to_add = {
                ...
                'user_creation': member.created_at.astimezone(UTC).strftime(
                    '%Y-%m-%d %H:%M:%S',
                ),
            }
            ...
            self.bot.logger.exception('Could not send welcome message')
    ...
    @group.command(
        name='toggle',
        description=app_commands.locale_str('command.welcome.toggle.description'),
    )
    @app_commands.describe(
        enabled=app_commands.locale_str('command.welcome.toggle.parameters.enabled'),
    )
    async def toggle_welcome(self, interaction: Interaction, enabled: bool = False):
        ...
        guild_settings = await self.bot.database.guild.get_config(guild_id)

        if (
            guild_settings.features
            and 'welcome' in guild_settings.features
            and isinstance(guild_settings.features['welcome'], WelcomeConfig)
        ):
    ...
    @group.command(
        name='joinchannel',
        description=app_commands.locale_str('command.welcome.joinchannel.description'),
    )
    @app_commands.describe(
        channel=app_commands.locale_str(
            'command.welcome.joinchannel.parameters.channel',
        ),
    )
    async def set_joinchannel(self, interaction: Interaction, channel: TextChannel):
        ...
        if (
            ...
            guild_settings.features['welcome'] = WelcomeConfig(
                channels=WelcomeChannels(join=channel.id),
            )
    ...
    @group.command(
        name='leavechannel',
        description=app_commands.locale_str('command.welcome.leavechannel.description'),
    )
    @app_commands.describe(
        channel=app_commands.locale_str(
            'command.welcome.leavechannel.parameters.channel',
        ),
    )
