...
class AdminOnlyCog(commands.Cog):
    ...
    async def safe_edit_message(self, message, embed: Embed, retries: int = 3):
        for _i in range(retries):
            try:
                ...
                has_ratelimited = isinstance(e, RateLimited) or (
                    isinstance(e, HTTPException) and e.status == 429
                )
                if has_ratelimited:
                    retry_after = getattr(e, 'retry_after', 2.0)
                    self.bot.logger.warning(
                        'Rate limited, retrying in %ss...',
                        retry_after,
                    )
    ...
    async def reload_cogs(
        ...
        targets = [
            e.replace('src.extensions.', '')
            for e in all_exts
            if not e.endswith('.admin')
        ]
        ...
        if last and last.result:
            ...
            final = Embed(
                ...
                color=Color.green() if last.failed == 0 else Color.red(),
            )
    ...
    @group.command(
        name='reboot',
        description=app_commands.locale_str('command.admin.reboot.description'),
    )
