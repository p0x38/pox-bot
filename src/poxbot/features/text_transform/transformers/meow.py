...
class MeowTransformer(BaseTextTransformer):
    ...
    _VARIANTS = np.array([("meow", ("m", "e", "o", "w")), ("miaw", ("m", "i", "a", "w")), ("maow", ("m", "a", "o", "w"))], dtype=object)
    _WEIGHTS = np.array([50.0, 2.0, 1.0])
    _WEIGHTS /= _WEIGHTS.sum()
    
    def _transform(self, request: TransformerRequest, *, context: TransformerContext | None = None) -> str:
