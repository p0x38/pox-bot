...
from src.managers.ai_provider import LLMManager, LLMProviderType
...
class LLMChatCog(commands.Cog):
    ...
    async def _save_history(self, channel_id: int):
        ...
        try:
            ...
            self.bot.logger.error(f"Failed to persist history for {channel_id}: {e}")
    ...
    async def format_discord_message(self, message: Message) -> dict[str, Any]:
        ...
        if message.reference and isinstance(message.reference.resolved, Message):
            ...
            ref_author = "You" if ref_msg.author == self.bot.user else ref_msg.author.display_name
            role_content = f"[{formatted_message_author} -> {ref_author}] {formatted_message_content}"
    ...
    async def populate_channel_cache(
            ...
            try:
                async with aiofiles.open(file_path, "rb") as f:
                    ...
                    if raw:
                        ...
                        for item in loaded:
                            self.history[channel_id].append(self._sanitize_history_item(item))
            except Exception as e:
                self.bot.logger.exception(f"Failed to load persisted history for {channel_id}: {e}")
        ...
        try:
            ...
            self.bot.logger.exception(f"Failed to populate channel cache: {e}")
            ...
            if cooldown_left.total_seconds() > 0:
                ...
                with contextlib.suppress(Exception):
                    return await message.reply(
                        f"You are on cooldown. Please retry after {remaining} seconds", allowed_mentions=AllowedMentions.none()
                    )

        if len(self.cooldowns) > 1000:
            self.cooldowns = {k: v for k, v in self.cooldowns.items() if now - v < self.cooldown_seconds}

        if message.channel.id not in self.history:
            await self.populate_channel_cache(message.channel)

        messages = [{"role": "system", "content": "You are a helpful assistant."}]
        current_msg_data = self._sanitize_history_item(await self.format_discord_message(message))

        if not self.history[message.channel.id] or self.history[message.channel.id][-1] != current_msg_data:
        ...
        async with lock:
            ...
            async with message.channel.typing():
                try:
                    ...
                    embed = Embed(title=e.__class__.__name__, description=str(e), color=Color.red())
                    self.bot.logger.exception(f"Failed to generate response: {e}")
                ...
                output = output.replace("{author_name}", message.author.display_name or "User")
                ...
                if len(send_content) > 2000:
                    chunks = [send_content[i : i + 2000] for i in range(0, len(send_content), 2000)]
                    for chunk in chunks:
                        if is_reply:
                            await message.reply(chunk, allowed_mentions=AllowedMentions.none())
                        else:
                            await message.channel.send(chunk, allowed_mentions=AllowedMentions.none())
                else:
                    if is_reply:
                        await message.reply(send_content, allowed_mentions=AllowedMentions.none())
                    else:
                        await message.channel.send(send_content, allowed_mentions=AllowedMentions.none())

                self.history[message.channel.id].append(self._sanitize_history_item({"role": "assistant", "content": output}))
                await self._save_history(message.channel.id)
    ...
    async def on_message(self, message: Message):
        ...
        if not is_triggered and hasattr(self, "is_matching") and self.is_matching(message.content):
            is_triggered = True
        ...
        await self.respond(
            message=message, provider=LLMProviderType.OPEN_ROUTER.value, model=self.bot.config.llm_config.model_id, include_history=read_history
        )
