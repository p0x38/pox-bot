...
class AdminOnlyCog(commands.Cog):
    ...
    def make_reload_embed(
        self,
        title: str, data: ExtensionProgress
    ):
        ...
        embed = Embed(
            description=(
                ...
                f"{data.current}/{data.total} ({percent:.1f}%)\n"
        ...
        for _i in range(retries):
            try:
                ...
                has_ratelimited = (
                    ...
                    or (isinstance(e, HTTPException) and e.status == 429)
                )
                if has_ratelimited:
    ...
    async def reload_cogs(
        ...
        targets = [e for e in all_exts if not e.endswith(".admin")]

        if not targets:
            return await interaction.followup.send(embed=Embed(
                description=self.bot.internal_translator.T('error.embeds.no_reloadable_extensions.description', )
            ))

        async for progress in manager.stream_operation(
            self.bot, ExtensionOperation.RELOAD,
            targets,
            concurrency=5,
            exclusions=["admin"],
        ):
