...
import tldextract
from discord import Color, Embed, Forbidden, HTTPException, Interaction, Message, NotFound, TextChannel, app_commands
from discord.ext import commands

from src.core.bot import PoxBot
from src.managers.spam import AntiSpamManager
from src.models.guild_settings_v2 import AntiSpamFilter
...
class GlobalChatCog(commands.Cog):
    ...
    def _record_counter(self, name: str, description: str, labels: dict[str, str]):
        if hasattr(self.bot, "metrics"):
            self.bot.metrics.increment_counter(name=name, description=description, amount=1, labels=labels)
    ...
    async def on_message(self, message: Message):
        ...
        if not global_chat or not global_chat.enabled or message.channel.id != global_chat.channel_id:
            return

        filtering = config.filtering
        antispam_setting = filtering.filters.get("anti_spam")

        if filtering.enabled and antispam_setting and isinstance(antispam_setting, AntiSpamFilter) and antispam_setting.enabled:
    ...
    async def handle_spam(self, message: Message):
        try:
            ...
            self.bot.logger.warning(
                f"Missing permissions to delete message in guild {message.guild.id if message.guild else 'Unknown'}"
            )
        except NotFound:
            pass

        embed = Embed(description=f"⚠️ {message.author.mention}, don't spam!", color=Color.red())
    ...
    async def censor_urls(self, text: str):
        ...
        found = self.bot.resources.url_extrator.find_urls(
            text, only_unique=True, check_dns=False, get_indices=True, with_schema_only=True
        )
    ...
    async def get_moderation_warning(self, flagged_data: dict, locale: str):
        ...
        if len(descriptions) == 1:
            combined = self.bot.internal_translator.T(
                "text.moderation.moderation.format.single", locale, {"content": descriptions[0]}
            )
        elif len(descriptions) == 2:
            combined = self.bot.internal_translator.T(
                "text.moderation.moderation.format.two", locale, {"first": descriptions[0], "last": descriptions[1]}
            )
        else:
            combined = self.bot.internal_translator.T(
        ...
        prefix = self.bot.internal_translator.T("text.moderation.flagged_message", locale, {"categories": combined})
        return prefix

    async def is_text_sendable(self, message: Message):
        if message.guild and self.bot.resources.profanity_filter.is_profane(message.content):
            return False
    ...
    async def broadcast_global_message(self, original: Message):
        ...
        if not await self.is_text_sendable(original):
            self._record_counter(
                name="bot_global_chat_messages_total",
                description="Total count of messages sent to the global chat system",
                labels={"guild_id": str(original.guild.id), "status": "filtered"}
            )
            return

        profile = await self.bot.database.user.get_full_profile(original.author.id) if self.bot.database.user else None
        display_name = getattr(profile, "nickname", original.author.display_name) if profile else original.author.display_name
        ...
        embed = Embed(description=await self.censor_urls(description_text), color=Color.green(), timestamp=original.created_at)
        ...
        self._record_counter(
            name="bot_global_chat_messages_total",
            description="Total count of messages sent to the global chat system",
            labels={"guild_id": str(original.guild.id), "status": "broadcasted"}
        )
        ...
        for guild in self.bot.guilds:
            ...
            config = await self.bot.database.guild.get_config(guild.id) if self.bot.database.guild else None
            if not config:
                continue
                ...
                if channel and isinstance(channel, TextChannel):
                    ...
                    try:
                        ...
                        self._record_counter(
                            ...
                            description="Total number of API errors encountered while relaying messages",
                            labels={"error_type": "forbidden", "target_guild": str(guild.id)}
                        )
                    except HTTPException as e:
                        self._record_counter(
                            name="bot_global_chat_broadcast_errors_total",
                            description="Total number of API errors encountered while relaying messages",
                            labels={"error_type": f"http_{e.status}", "target_guild": str(guild.id)}
                        )
        ...
        if hasattr(self.bot, "metrics"):
            self.bot.metrics.record_histogram(
                ...
                description="The time taken to relay a single global chat message to all connected guilds in seconds",
    ...
    @group.command(name="setup", description=app_commands.locale_str("command.global_chat.setup.description"))
    @app_commands.describe(channel=app_commands.locale_str("command.global_chat.setup.parameters.channel"))
    ...
    async def globalchat_setup(self, interaction: Interaction, channel: TextChannel):
        ...
        if not interaction.guild or not self.bot.database.guild:
            return await interaction.followup.send(
                self.bot.internal_translator.T("error.embeds.database_not_available.description", loc),
            )
        ...
        await interaction.followup.send(
            self.bot.internal_translator.T("command.global_chat.setup.embeds.default.description", loc, channel=channel.mention)
        )

    @group.command(name="toggle", description=app_commands.locale_str("command.global_chat.toggle.description"))
    ...
    async def globalchat_toggle(self, interaction: Interaction):
        ...
        if not config.global_chat.channel_id:
            return await interaction.followup.send(
                self.bot.internal_translator.T("error.embeds.feature_not_available.description", loc)
            )
        ...
        status_text = self.bot.internal_translator.T(
            "text.boolean.true" if config.global_chat.enabled else "text.boolean.false", loc
        )
        await interaction.followup.send(
            self.bot.internal_translator.T(
                "command.global_chat.toggle.embeds.default.description", loc, status_text=status_text
            )
        )

    @group.command(name="status", description=app_commands.locale_str("command.global_chat.status.description"))
    async def globalchat_status(self, interaction: Interaction):
        ...
        channel_mention = f"<#{gc.channel_id}>" if gc.channel_id else self.bot.internal_translator.T("text.unset", loc)
        status_text = self.bot.internal_translator.T(
        ...
        embed = Embed(
            title=self.bot.internal_translator.T(
                "command.global_chat.status.embeds.default.title", loc, guild_name=interaction.guild.name
            ),
            color=Color.blue(),
        )
        embed.add_field(name=self.bot.internal_translator.T("label.global_chat_status", loc), value=status_text, inline=False)
        embed.add_field(
            name=self.bot.internal_translator.T("label.global_chat_channel", loc), value=channel_mention, inline=False
        )
        ...
        if filtering and filtering.enabled:
            ...
            if antispam and antispam.enabled and isinstance(antispam, AntiSpamFilter):
                embed.add_field(
                    name=self.bot.internal_translator.T("label.global_chat_antispam", loc),
                    value=self.bot.internal_translator.T(
                        "command.global_chat.status.labels.anti_spam",
                        loc,
                        {"per": antispam.window_length, "count": antispam.messages_per_window},
                    ),
                    inline=False,
                )
