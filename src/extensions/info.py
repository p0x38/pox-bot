...
class FeedbackModal(ui.Modal):
    ...
    async def on_error(self, interaction: Interaction, error: Exception) -> None:  # noqa: ARG002
        self.bot.logger.exception("Exception thrown while trying to process the submission")
...
class DynamicInfoView(ui.View):
    def __init__(self, cog: "InfoCog", bot: PoxBot, locale: Locale | str):
        ...
        url_button = ui.Button(
            label=self.bot.internal_translator.T("modal.DynamicInfoView.buttons.url_button.label", locale),
    ...
    async def select_callback(self, interaction: Interaction, select: Select):
        ...
        for _, field_info in category_data["fields"].items():
            embed.add_field(name=field_info['display'], value=field_info['value'], inline=True)

        await interaction.edit_original_response(embed=embed, view=self)

    @ui.button(style=ButtonStyle.primary)
    async def feedback_button(self, interaction: Interaction, button: ui.Button):  # noqa: ARG002
    ...
    async def get_stats_data(self, interaction: Interaction):
        loc = (
            ...
            if (hasattr(self.bot, 'database') and self.bot.database and self.bot.database.settings) else interaction.locale
        )

        cpu_usage, memory_info, disk_usage, process_cpu, process_memory = await asyncio.gather(
            ...
            "identity": {
                ...
                "fields": {
                    ...
                    "version": {
                        ...
                        "value": (
                            f"git+{self.bot.git_info.commit_hash[:7]
                            if self.bot.git_info.commit_hash
                            else 'No commit hash found'}"
                            f"{self.bot.git_info.commit_message
                    ...
                    "signature": {
                        ...
                        "value": self.bot.statistics.session_signature or "Unknown signature"
                    },
                    "uptime": {
                        "display": self.bot.internal_translator.T(
            ...
            "stats": {
                ...
                "fields": {
                    ...
                    "interactions": {
                        ...
                        "value": f"P: {self.bot.statistics.interaction_statistics.total} | F: {
                            self.bot.statistics.interaction_statistics.failed}"
                    },
                    "cached_values": {
                        "display": self.bot.internal_translator.T(
            ...
            "hardware": {
                ...
                "fields": {
                    ...
                    "ram_details": {
                        ...
                        "value": f"{memory_info.used // (1024**2)}MB / {memory_info.total // (1024**2)}MB"
                    }
                }
            }
        }
...
class InfoCog(commands.Cog):
    ...
    async def ping_bot(self, interaction: Interaction):
        ...
        embed = Embed(
            title=self.bot.i18n_manager.T(
                "command.info.ping.embeds.default.title", loc,
                latency=str(round(self.bot.latency * 10000) / 100)
            )
        )

        rows_to_add = {
            'Shard ID': self.bot.shard_id or self.bot.i18n_manager.T("text.standalone", loc),
            'Shards': self.bot.shard_count or self.bot.i18n_manager.T("text.standalone", loc)
        }
    ...
    async def invite(self, interaction: Interaction):
        try:
            ...
            embed = Embed(
                title=self.bot.i18n_manager.T("command.info.invite.embeds.default.title", loc),
                description=self.bot.i18n_manager.T("command.info.invite.embeds.default.description", loc,
                                   {"invite_url": invite_url}),
                color=Color.red() if guild_count >= limit else Color.blurple()
            )

            if guild_count >= limit:
                embed.description = self.bot.i18n_manager.T("command.info.invite.error.hardlimited", loc)

            embed.set_footer(text=self.bot.i18n_manager.T("command.info.invite.embeds.default.footer", loc))
