...
class ContributorsCog(commands.Cog):
    ...
    async def cog_load(self) -> None:
        try:
            self.contributors_list = await self.bot.resources.load_contributors_async("contributors.v2.json")

            self.bot.logger.info("Successfully pre-loaded contributor data in cog_load")
        except FileNotFoundError:
            self.bot.logger.error("contributors.v2.json doesn't seems exists. returning empty")
            self.contributors_list = []

    @group.command(name="list", description=app_commands.locale_str("command.contributors.list.description"))
    async def list_contributors(self, interaction: Interaction):
        loc = await self.bot.get_locale(interaction)

        embed = Embed(
            title=self.bot.internal_translator.T("command.contributors.list.embeds.default.title", str(loc)),
            description=self.bot.internal_translator.T("command.contributors.list.embeds.default.description", str(loc)),
        )
        ...
        for contributor in self.contributors_list:
            ...
            name = user.display_name if user else contributor.command or self.bot.internal_translator.T("text.unknown", loc)

            contribution = contributor.description.default or self.bot.internal_translator.T("text.unknown", str(loc))
    ...
    async def contributor_autocomplete(self, interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:  # noqa: ARG002
        return [
    ...
    @group.command(name="view", description=app_commands.locale_str("command.contributors.view.description"))
    ...
    async def view_contributor(self, interaction: Interaction, contributor_id: str):
        ...
        contributor_data = next((d for d in self.contributors_list if d.command == contributor_id), None)

        if contributor_data:
            is_scary_active = hasattr(self.bot, "constants") and getattr(self.bot.constants, "scary_mode", False)
            ...
            if is_scary_active:
                final_title = self.bot.text_converter.get_scary_text(final_title)
                if not contributor_data.description.scary and not contributor_data.content.scary:
                    final_description = self.bot.text_converter.get_scary_text(final_description)
                if not contributor_data.quote.scary and final_footer:
                    final_footer = self.bot.text_converter.get_scary_text(final_footer)

            embed = Embed(title=final_title, color=Color.blue(), description=final_description)
            if final_footer:
                embed.set_footer(text=final_footer)

            pic = None
            if chosen_thumbnail and isinstance(chosen_thumbnail, str) and chosen_thumbnail.startswith("file:"):
                ...
                try:
                    ...
                    self.bot.logger.warning(f"Contributor asset file missing: {relative_file_path}")
            ...
            error_embed.title = self.bot.internal_translator.T("error.embeds.contributor_not_found.title", str(loc))
            error_embed.description = self.bot.internal_translator.T(
                "error.embeds.contributor_not_found.description", str(loc), {"contributor": contributor_id}
            )
