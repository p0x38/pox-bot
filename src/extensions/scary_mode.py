...
class ScaryModeCog(commands.Cog):
    def __init__(self, bot: PoxBot):
        ...
        self._cooldown = commands.CooldownMapping.from_cooldown(1, self.cooldown_time, commands.BucketType.default)
        ...
        if self.bot.user in message.mentions:
            ...
            if self.scary_pattern.match(cleaned_content):
                ...
                if hasattr(self.bot, "constants"):
                    ...
                    self.bot.logger.warning(f"Scary mode toggled on by {message.author.display_name}")

                asyncio.create_task(self._reset_scary_mode_after_delay(self.scary_duration))
