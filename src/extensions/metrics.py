...
class OpenTelemetryMetrics(commands.Cog):
    ...
    async def _instrumented_tree_error(self, interaction: Interaction, error: app_commands.AppCommandError):
        original_error = getattr(error, "original", None)
        error_name = error.__class__.__name__

        if original_error is not None:
            full_error_string = f"{error.__class__.__name__}:{original_error.__class__.__name__}"
        else:
    ...
    async def on_interaction(self, interaction: Interaction):
        ...
        cmd_name = interaction.command.qualified_name if interaction.command else "unknown"
        if interaction.type in (InteractionType.component, InteractionType.modal_submit):
            cmd_name = interaction.data.get("custom_id", "unknown_component") if interaction.data else "unknown_component"
        ...
        if interaction.type != InteractionType.application_command:
            asyncio.create_task(
                self._track_generic_interaction_duration(interaction, interaction_type_str, cmd_name, start_time)
            )

    async def _track_generic_interaction_duration(self, interaction: Interaction, type_str: str, name: str, start_time: float):
        attempt = 0
        while attempt < 30:
        ...
        self.bot.metrics.record_histogram(
            ...
            description="The execution duration of all received interactions in seconds",
            value=duration,
            labels={
                "type": type_str,
                "command_or_id": name,
    ...
    @commands.Cog.listener()
    async def on_app_command_completion(self, interaction: Interaction, command):  # noqa: ARG002
        if not command:
            return
        ...
        if start:
            ...
            self.bot.metrics.record_histogram(
                ...
                description="The execution duration of application commands in seconds",
                value=duration,
                labels={
    ...
    async def calculate_loop_lag(self):
        ...
        self.bot.metrics.set_gauge(
            "bot_event_loop_lag_seconds", "The current event loop execution lag in seconds", lag_to_record
        )

    async def record_latency_per_shard(self):
        for id, latency in self.bot.latencies:
            if math.isinf(latency) or math.isnan(latency):
                continue

            self.bot.metrics.set_gauge(
                name="bot_shard_latency_seconds",
                description="The current WebSocket latency to the Discord API Gateway in seconds per shard",
                value=float(latency),
                labels={
                    "shard_id": str(id),
                },
    ...
    async def update_all_gauges(self):
        ...
        self.bot.metrics.set_gauge(
            "bot_guilds", "The total number of Discord guild the bot is currently connected to", len(self.bot.guilds)
        )
        self.bot.metrics.set_gauge(
            "bot_users", "The total number of unique users visible to the bot across all shards", len(self.bot.users)
        )
        self.bot.metrics.set_gauge(
            "bot_shards", "The total number of shards configured for this bot instance", self.bot.shard_count or 1
        )
