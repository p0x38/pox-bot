    ...
    async def load(self):
        ...
        async with self.lock:
            try:
                ...
                logger.exception(
                    f'Error loading ticket data: {e}. Starting with empty data'
                )
                self.data = {}
...
class TicketCloseButton(discord.ui.View):
    ...
    @discord.ui.button(
        label='Close ticket', style=ButtonStyle.red, custom_id='persistent_ticket_close'
    )
...
class TicketPanel(discord.ui.View):
    ...
    async def create_button_callback(
...
class TicketSystemCog(commands.Cog):
    ...
    async def process_ticket_creation(self, interaction: Interaction, reason: str):
        if interaction.guild is None:
            return await interaction.response.send_message(
                'The ticket system cannot be used in User-mode.'
            )
        ...
        if not category or not isinstance(category, CategoryChannel):
            return await interaction.followup.send(
                'Error: ticket category not found.', ephemeral=True
            )
        ...
        ticket_channel_name = (
            f'ticket-{member.name.lower()[:10]}-{random.randint(0, 9999)}'
        )
        try:
            new_channel = await guild.create_text_channel(
                ticket_channel_name, category=category, overwrites=overwrites
            )
        except Forbidden:
            return await interaction.followup.send(
                "I don't have permission to create ticket >:(", ephemeral=True
            )
        ...
        await new_channel.send(
            f'{member.mention}', embed=embed, view=TicketCloseButton(self)
        )

        await interaction.followup.send(
            f'Your ticket has been created. Head over to {new_channel.mention} :3',
            ephemeral=True,
        )
    ...
    async def close_ticket(self, interaction: Interaction):
        ...
        if not isinstance(channel, TextChannel):
            return await interaction.response.send_message('Failed to close ticket.')
        ...
        if not isinstance(interaction.user, Member):
            return await interaction.response.send_message('The user is not member.')

        is_manager = any(r.id in manager_role_ids for r in interaction.user.roles)

        ticket_info = guild_data['tickets'].get(str(channel.id))
        is_creator = (
            ticket_info and int(ticket_info.get('user_id')) == interaction.user.id
        )
        ...
        await interaction.response.send_message(
            'Closing the ticket in 5 seconds... Have a great day!', ephemeral=False
        )
    ...
    @group.command(
        name='setup', description='Configures the ticket system. (Work in progress)'
    )
    @app_commands.default_permissions(administrator=True)
    async def ticket_setup(
        self,
        interaction: Interaction,
        panel_chanel: TextChannel,
        ...
        if guild is None:
            return await interaction.response.send_message(
                'You cannot use setup in out of guild-mode.'
            )
        ...
        embed = Embed(
            ...
            description='Need help? click the button below to open a new private support. (Work in progress)',
            color=Color.gold(),
        )
