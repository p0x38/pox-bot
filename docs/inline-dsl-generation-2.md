...
* **What is it?** A safe, custom mini-programming language to glitch, filter, and hack text.
* **How does it work?** It looks at your text **one character at a time**.
* **What do I do?** You write a short, simple rule (like `'upper' if is_vowel else 'keep'`).
* **The Result:** The bot processes every character based on your rule and spits out the modified text instantly!

---

## 👶 The "Explain Like I'm 5" Guide (Super Easy!)

Imagine your text is a line of people waiting to get into a club, and your DSL rule is the **Bouncer**.
The Bouncer looks at each person (character) **one by one** and asks: *"What should I do with you?"*

* If the Bouncer says `True` ➡ The person goes inside normally. (`'keep'`)
* If the Bouncer says `False` ➡ The person gets kicked out! (`'delete'`)
* If the Bouncer says `'upper'` ➡ The person is forced to wear a giant hat! (Capitalized)
* If the Bouncer says `'💥'` ➡ The person magically turns into an explosion!

**Let's trace a real example!**
Imagine the input text is: `c a t`
Your rule is: `'delete' if is_vowel else 'keep'`

1. The Bouncer looks at `c`: *"You are not a vowel. KEEP!"* ➡ `c`
2. The Bouncer looks at `a`: *"You are a vowel! DELETE!"* ➡ (gone)
3. The Bouncer looks at `t`: *"You are not a vowel. KEEP!"* ➡ `t`

**Final Output:** `ct`
See? It's that simple! You are just giving the Bouncer a rule to follow!
