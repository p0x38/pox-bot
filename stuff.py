...
def get_postgresql_dsn():
    ...
    if any(v is None for v in [user, password, database_name, host]):
        required = [i for i, v in {"POSTGRESQL_USER": user, "POSTGRESQL_PASS": password, "POSTGRESQL_DATABASE": database_name,
                                   "POSTGRESQL_HOST": host}.items() if v is None]
...
def _find_key_recursive(config: dict, key) -> bool:
    ...
    return any(isinstance(value, dict) and _find_key_recursive(value, key) for value in config.values())
...
def approach_target(target: float, max_iterations: int = 125, x: float = 1.75, current_range: tuple = (-5, 5),
                    step_varience: tuple = (.5, 2)):
...
async def get_markov_dataset(name: str = "2"):
    if not os.path.exists(f"./src/assets/{name}.txt"):
        logger.error("The file not found")
        return None
    async with aiofiles.open(f"./src/assets/{name}.txt", encoding="utf-8") as f:
        line = await f.read()
