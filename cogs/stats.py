...
class StatsCog(commands.Cog):
    ...
    async def wordcloud(self, interaction: Interaction, limit: int = 1000):
        ...
        if not interaction.channel:
            return await interaction.followup.send(
                embed=Embed(
                    title=i18n.T("error.embeds.unsupported_channel_type.title", loc),
                    description=i18n.T("error.embeds.unsupported_channel_type.description", loc),
                    color=Color.red(),
                    timestamp=datetime.now(UTC)
                )
            )
        ...
        if not rows:
            return await interaction.followup.send(
                embed=Embed(
                    title=i18n.T("error.embeds.data_not_found.title", loc),
                    description=i18n.T("error.embeds.data_not_found.description", loc),
                    color=Color.red(),
                    timestamp=datetime.now(UTC)
                )
            )
